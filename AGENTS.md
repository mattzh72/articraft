...
Supported providers are `openai`, `gemini`, `anthropic`, `openrouter`, and `codex-cli`. Use `model_id=codex-cli-default` for no-key Codex CLI rows unless a specific Codex model is requested. `thinking_level` must be `low`, `med`, or `high`; `max_turns` must be a positive integer. If `row_id` is omitted, rows default to `row_0001`, `row_0002`, and so on, but stable explicit IDs are better for resume. `image_path` is intentionally unsupported in batch CSV v1. For new batch specs, leave per-row cost caps blank unless explicitly requested and use `max_turns=100` by default.
