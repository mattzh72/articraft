...
- Use `provider=codex-cli` with an explicit Codex model ID for no-key Codex generation through the internal Articraft harness. Turn/tool/compile trajectory remains available; total token count is recorded when Codex CLI emits its `tokens used` line, but billable cost remains unavailable.
