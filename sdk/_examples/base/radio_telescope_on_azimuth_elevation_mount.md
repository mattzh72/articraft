---
title: 'Radio Telescope on Azimuth-Elevation Mount'
description: 'Base SDK radio telescope example with a large lathed dish, feed horn, yoke arms, azimuth pedestal, elevation trunnion, structural braces, and observatory-grade support geometry.'
tags:
  - sdk
  - base sdk
  - radio telescope
  ...
  - mount structure
---
# Radio Telescope on Azimuth-Elevation Mount

This base-SDK example is a strong reference for a realistic observatory radio telescope with a large dish, substantial azimuth-elevation mount, yoke arms, feed structure, pedestal base, and counterweighting. It is useful for queries such as `radio telescope`, `dish antenna`, `parabolic dish`, `azimuth elevation mount`, `az-el mount`, `yoke mount`, `pedestal`, `feed horn`, `lathe geometry`, `torus geometry`, `sweep_profile_along_spline`, and `tube_from_spline_points`.

The modeling patterns worth copying are:

- `LatheGeometry(...)` and `TorusGeometry(...)` for the dish shell, rim, and rear reinforcement rings.
- `sweep_profile_along_spline(...)` for heavy yoke arms and spine members with controlled cross-sections.
- `tube_from_spline_points(...)` for braces, ribs, and feed-support struts.
- a two-axis articulated mount with continuous azimuth and revolute elevation motion.
- layered structural detailing for pedestal, bearing housing, yoke, dish back ribs, feed cabin, and counterweight box.
